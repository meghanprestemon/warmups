...
citiesOffered = ['Stockholm'];

// conferencePicker (citiesVisited, citiesOffered);
// // ---> 'Paris'

function conferencePicker (citiesVisited, citiesOffered) {
  let conferenceChoice = citiesOffered.filter(city => !citiesVisited.includes(city));

  if (conferenceChoice[0] !== undefined) {
    return conferenceChoice[0];
  }
